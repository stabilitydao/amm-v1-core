...
describe("MasterChefV2", function () {
  ...
  describe("Init", function () {
    it("Balance of dummyToken should be 0 after init(), repeated execution should fail", async function () {
      await expect(this.chief2.init(this.dummy.address)).to.be.revertedWith("Balance must exceed 0")
    })
  })
  ...
  describe("Set", function () {
    it("Should emit event LogSetPool", async function () {
      ...
      await expect(this.chief2.set(0, 10, this.dummy.address, true)).to.emit(this.chief2, "LogSetPool").withArgs(0, 10, this.dummy.address, true)
    })
  ...
  describe("PendingReact", function () {
    it("PendingReact should equal ExpectedReact", async function () {
      ...
      await advanceBlock()
      let expectedReact = getBigNumber(100)
  ...
  describe("UpdatePool", function () {
    it("Should emit event LogUpdatePool", async function () {
      ...
      await expect(this.chief2.updatePool(0))
        ...
        .withArgs(
          0,
          (await this.chief2.poolInfo(0)).lastRewardBlock,
    ...
    it("Should take else path", async function () {
      ...
      await this.chief2.batch(
        [this.chief2.interface.encodeFunctionData("updatePool", [0]), this.chief2.interface.encodeFunctionData("updatePool", [0])],
        true
      )
    })
  })
  ...
  describe("Harvest", function () {
    it("Should give back the correct amount of REACT and reward", async function () {
      ...
      expect(await this.react.balanceOf(this.alice.address))
        .to.be.equal(await this.r.balanceOf(this.alice.address))
        .to.be.equal(expectedReact)
    })
