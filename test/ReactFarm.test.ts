...
describe("ReactFarm", function () {
  ...
  it("should set correct state variables", async function () {
    ...
    // todo check roles of REACT instead this
  ...
  context("With ERC/LP token added to the field", function () {
    ...
    // todo test for "deposit REACT by staking"
    ...
    it("should allow emergency withdraw", async function () {
      ...
      await this.chief.connect(this.bob).deposit(1, "100")

      expect(await this.lp.balanceOf(this.bob.address)).to.equal("900")

      await this.chief.connect(this.bob).emergencyWithdraw(1)
    ...
    it("should give out REACTs only after farming time", async function () {
      ...
      await this.chief.connect(this.bob).deposit(1, "0") // block 90
      expect(await this.react.balanceOf(this.bob.address)).to.equal("0")
      await advanceBlockTo("95")
      await this.chief.connect(this.bob).deposit(1, "0") // block 95
      expect(await this.react.balanceOf(this.bob.address)).to.equal("0")
      await advanceBlockTo(currentBlock + 95)
      await this.chief.connect(this.bob).deposit(1, "0") // block 100
      expect(await this.react.balanceOf(this.bob.address)).to.equal("0")
      await advanceBlockTo(currentBlock + 100)
      await this.chief.connect(this.bob).deposit(1, "0") // block 101
      expect(await this.react.balanceOf(this.bob.address)).to.equal("1000")
      await advanceBlockTo(currentBlock + 104)
      await this.chief.connect(this.bob).deposit(1, "0") // block 105
    ...
    it("should not distribute REACTs if no one deposit", async function () {
      ...
      await this.chief.connect(this.bob).deposit(1, "10") // block 210
      expect(await this.react.totalSupply()).to.equal("0")
      expect(await this.react.balanceOf(this.bob.address)).to.equal("0")
      ...
      await this.chief.connect(this.bob).withdraw(1, "10") // block 220
    ...
    it("should distribute REACTs properly for each staker", async function () {
      ...
      await this.chief.connect(this.bob).deposit(1, "20", { from: this.bob.address })
      ...
      await this.chief.connect(this.bob).withdraw(1, "5", { from: this.bob.address })
      ...
      await this.chief.connect(this.alice).withdraw(1, "20", { from: this.alice.address })
      await advanceBlockTo(currentBlock + 349)
      await this.chief.connect(this.bob).withdraw(1, "15", { from: this.bob.address })
      await advanceBlockTo(currentBlock + 359)
      await this.chief.connect(this.carol).withdraw(1, "30", { from: this.carol.address })
    ...
    it("should give proper REACTs allocation to each pool", async function () {
      ...
      expect(await this.chief.pendingReact(1, this.alice.address)).to.equal("10000")
      // Bob deposits 10 LP2s at block 425
      await advanceBlockTo(currentBlock + 424)
      await this.chief.connect(this.bob).deposit(2, "5", { from: this.bob.address })
      // Alice should have 10000 + 5*1/3*1000 = 11666 pending reward
      expect(await this.chief.pendingReact(1, this.alice.address)).to.equal("11666")
      await advanceBlockTo(currentBlock + 430)
      // At block 430. Bob should get 5*2/3*1000 = 3333. Alice should get ~1666 more.
      expect(await this.chief.pendingReact(1, this.alice.address)).to.equal("13333")
      expect(await this.chief.pendingReact(2, this.bob.address)).to.equal("3333")
    })
    ...
    it("should stop giving bonus REACTs after the bonus period ends", async function () {
      ...
      expect(await this.chief.pendingReact(1, this.alice.address)).to.equal("10500")
      // At block 606, Alice withdraws all pending rewards and should get 10600.
      await this.chief.connect(this.alice).deposit(1, "0", { from: this.alice.address })
      expect(await this.chief.pendingReact(1, this.alice.address)).to.equal("0")
