...
contract CloneRewarderTimeDual is IRewarder, BoringOwnable {
    ...
    function init(bytes calldata data) public payable {
        ...
        (
            ...
            data,
            (IERC20, IERC20, address, uint128, uint128, IERC20)
        );
    ...
    function onReactReward(
        ...
        if (_userInfo.amount > 0) {
            pending1 = (_userInfo.amount.mul(pool.accToken1PerShare) /
                ACC_TOKEN_PRECISION)
                .sub(_userInfo.rewardDebt1);
            pending2 = (_userInfo.amount.mul(pool.accToken2PerShare) /
                ACC_TOKEN_PRECISION)
                .sub(_userInfo.rewardDebt2);
        ...
        _userInfo.rewardDebt2 =
            lpTokenAmount.mul(pool.accToken2PerShare) /
            ACC_TOKEN_PRECISION;
    ...
    function setRewardPerSecond(
        uint128 _rewardPerSecond1,
        uint128 _rewardPerSecond2
    ) public onlyOwner {
        ...
        returns (uint256 reward1, uint256 reward2)
    {
        ...
        if (block.timestamp > pool.lastRewardTime && lpSupply != 0) {
            ...
            accToken1PerShare = accToken1PerShare.add(
                pending1.mul(ACC_TOKEN_PRECISION) / lpSupply
            );
            accToken2PerShare = accToken2PerShare.add(
                pending2.mul(ACC_TOKEN_PRECISION) / lpSupply
            );
        }
        reward1 = (user.amount.mul(accToken1PerShare) / ACC_TOKEN_PRECISION)
            .sub(user.rewardDebt1);
        reward2 = (user.amount.mul(accToken2PerShare) / ACC_TOKEN_PRECISION)
        ...
        if (block.timestamp > pool.lastRewardTime) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accToken1PerShare = pool.accToken1PerShare.add(
                    (pending1.mul(ACC_TOKEN_PRECISION) / lpSupply).to128()
                );
                pool.accToken2PerShare = pool.accToken2PerShare.add(
                    (pending2.mul(ACC_TOKEN_PRECISION) / lpSupply).to128()
                );
            }
