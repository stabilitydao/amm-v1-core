...
contract MasterChefV2 is BoringOwnable, BoringBatchable {
    ...
    uint256 private constant MASTERCHEF_REACT_PER_BLOCK = 1e20;
        ...
        returns (uint256 pending)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 reactReward =
                blocks.mul(reactPerBlock()).mul(pool.allocPoint) /
                    totalAllocPoint;
    ...
    function reactPerBlock() public view returns (uint256 amount) {
        amount =
            uint256(MASTERCHEF_REACT_PER_BLOCK).mul(
                REACT_MASTER.poolInfo(MASTER_PID).allocPoint
            ) /
            REACT_MASTER.totalAllocPoint();
    }
